package idat.pe.app_auth_service.config.jwt;
...
import idat.pe.app_auth_service.service.UserDetailsImpl;
...
public class JwtUtils {
    ...
    public boolean validateJwtToken(String token) {
        ...
        } catch (ExpiredJwtException e) { logger.error("Token Expirado: {}", e.getMessage());
        } catch (IllegalArgumentException e) { logger.error("Permisos vacios: {}", e.getMessage());
        } catch (UnsupportedJwtException e) { logger.error("Token no soportado: {}", e.getMessage()); }
        return false;
    }
}
